<template>
  <div id="register">
    ...
    <transition appear enter-active-class="animated lightSpeedIn" leave-active-class="animated rotateInUpLeft" :duration="3000">
      <div class="wrap">
        <van-cell-group>
          ...
          <van-field v-model="password" type="password" label="密码" placeholder="请输入密码" required error />
  ...
  methods: {
    ...
    register() {
      if (this.phone == '' || this.password == '' || this.nickname == '' || this.captcha == '') {
        ...
        // 验证手机号是否注册过
        this.axios.get('/cellphone/existence/check?phone=' + this.phone).then((res) => {
          if (res.data.exist == 1 || res.data.exist == 2) {
            ...
            // 验证验证码
            this.axios.get('/captcha/verify?phone=' + this.phone + '&captcha=' + this.captcha).then((res) => {
              if (res.data.code == !200) {
                this.$dialog.alert({
                ...
                // 注册
                this.axios.get('/register/cellphone?phone=' + this.phone + '&password=' + this.password + '&captcha=' + this.captcha + '&nickname=' + this.nickname).then((res) => {
                  console.log(res)
                  this.$dialog
                    .alert({
                      message: '注册账号成功！'
